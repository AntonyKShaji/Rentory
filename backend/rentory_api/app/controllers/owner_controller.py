...
def list_properties(owner_id: str, db: Session = Depends(get_db), current_user: AuthUser = Depends(get_current_user)) -> list[PropertyCardResponse]:
...
def owner_analytics(owner_id: str, db: Session = Depends(get_db), current_user: AuthUser = Depends(get_current_user)) -> OwnerAnalyticsResponse:
    enforce_user_scope(current_user, owner_id, "owner")
